...
import { reconstructArborFile } from '../../utils/editor-engine.js';

const EMOJIS = ["📄", "📁", "📂", "✨", "🔥", "💡", "🚀", "⭐", "📝", "📌", "🧬", "💻", "🎨", "⚖️", "🧠", "🎓", "🌱", "🌳", "🍎", "🎮"];

class ArborModalNodeProperties extends HTMLElement {
    constructor() {
        super();
    ...
    async save() {
        ...
        try {
            ...
            if (isFolder) {
                ...
                newContent = reconstructArborFile(newMeta, originalBody);
            }
    ...
    render() {
        if (this.state.loading) {
            this.innerHTML = `<div class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm"><div class="animate-spin text-4xl">⏳</div></div>`;
            return;
        }
        ...
        <div id="modal-backdrop" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-xl w-full relative overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 cursor-auto transition-all duration-300">
                ...
                <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                    ...
                    <button class="btn-close w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 transition-colors">✕</button>
                </div>
                ...
                <div class="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex gap-3">
                    <button class="btn-close flex-1 py-3 text-slate-500 dark:text-slate-400 font-bold text-sm hover:text-slate-800 dark:hover:text-white transition-colors">Cancel</button>
...
customElements.define('arbor-modal-node-properties', ArborModalNodeProperties);
