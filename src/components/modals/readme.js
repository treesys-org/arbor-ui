...
class ArborModalReadme extends HTMLElement {
    ...
    renderSkeleton() {
        ...
        <div id="readme-backdrop" class="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/80 backdrop-blur-md p-4 animate-in fade-in duration-500">
            <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl max-w-3xl w-full relative overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 cursor-auto transition-all duration-300 h-[650px] max-h-[90vh]">
                
                <!-- Hero Image / Cover (Fixed Height) -->
                <div class="bg-gradient-to-br from-green-400 to-blue-500 h-24 md:h-32 shrink-0 relative flex items-center justify-center">
                    <button class="btn-close absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-white/20 hover:bg-white/40 text-white transition-colors backdrop-blur-md z-10">✕</button>
                    <div class="w-20 h-20 md:w-24 md:h-24 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center text-4xl md:text-5xl shadow-xl border-4 border-white dark:border-slate-800 absolute -bottom-10 md:-bottom-12">
                        ${icon}
                    </div>
                </div>

                <!-- Header (Shrink-0) -->
                <div class="px-6 md:px-8 pt-14 md:pt-16 pb-0 text-center shrink-0">
                    <h2 class="text-xl md:text-3xl font-black text-slate-800 dark:text-white mb-2 leading-tight">${title}</h2>
                    <div class="w-12 h-1 bg-slate-200 dark:bg-slate-700 mx-auto rounded-full mb-4 md:mb-6"></div>
                </div>

                <!-- Content Body (Flex-1 + Scroll) -->
                <div id="readme-body" class="px-6 md:px-8 pb-6 flex-1 overflow-y-auto custom-scrollbar min-h-0">
