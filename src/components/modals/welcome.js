...
class ArborModalWelcome extends HTMLElement {
    ...
    getMobileProgressHtml(steps) {
        return `
        <div class="md:hidden flex gap-1 p-4 pb-0 shrink-0">
            ${steps.map((_, i) => `
    ...
    getContentHtml(current, ui) {
        ...
        <div class="flex-1 p-8 md:p-12 flex flex-col justify-center items-center text-center overflow-y-auto custom-scrollbar animate-in slide-in-from-right-4 fade-in duration-300" key="${this.activeStep}">
            <div class="w-24 h-24 md:w-32 md:h-32 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-6xl md:text-7xl mb-8 shadow-sm border border-slate-100 dark:border-slate-700 animate-in zoom-in duration-500 shrink-0">
                ${current.icon}
            </div>
    ...
    bindDynamicEvents() {
        ...
        // Re-bind skip button (if rendered in footer)
        this.querySelectorAll('.btn-close').forEach(b => b.onclick = () => this.close());
    }
    ...
    render() {
        ...
        // Key to detect full re-render needs (Lang/Theme change)
        const renderKey = `${lang}-${theme}`;
        
        // --- PARTIAL UPDATE LOGIC ---
        // If the main structure exists and theme/lang hasn't changed, we just update the dynamic zones.
        const contentContainer = this.querySelector('#welcome-content-area');
        
        if (contentContainer && this.lastRenderKey === renderKey) {
            // 1. Update Sidebar List
            const sidebarList = this.querySelector('#sidebar-list');
            if (sidebarList) sidebarList.innerHTML = this.getSidebarItemsHtml(steps);
            ...
            if(footerContainer) footerContainer.innerHTML = this.getFooterHtml(ui, total, isLast);
            
            // Re-bind events for the newly injected buttons
        ...
        <div id="modal-backdrop" class="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in">
            ...
            <div class="bg-slate-50 dark:bg-slate-900 rounded-[24px] shadow-2xl w-full h-auto max-h-[90vh] relative overflow-hidden flex flex-col md:flex-row border border-slate-200 dark:border-slate-800 transition-all duration-300">
                ...
                <div class="hidden md:flex w-1/3 bg-slate-50/50 dark:bg-slate-950/50 border-r border-slate-200 dark:border-slate-800 flex-col p-6 shrink-0">
                    <div class="mb-6 pl-2">
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 text-center shrink-0">
                        <p class="text-[10px] text-slate-400">Powered by Treesys</p>
                    </div>
                </div>

                <!-- RIGHT CONTENT (Page) -->
                <div class="flex-1 flex flex-col relative bg-white dark:bg-slate-900 h-full min-w-0">
