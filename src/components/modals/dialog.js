...
class ArborModalDialog extends HTMLElement {
    constructor() {
        super();
        this.value = '';
    }
    ...
    render() {
        ...
        this.innerHTML = `
        <div id="modal-backdrop" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 relative overflow-hidden">
                
                <div class="flex flex-col items-center text-center mb-6">
                    <div class="text-4xl mb-4">${icon}</div>
                    <h3 class="text-lg font-black text-slate-800 dark:text-white mb-2">${title}</h3>
...
customElements.define('arbor-modal-dialog', ArborModalDialog);
